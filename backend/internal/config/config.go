...
// IsBuiltInPIACP reports whether an agent uses the exact PI adapter command
// generated by Lumi. This narrow match is the only configuration redirected
// to Lumi's embedded bridge; custom commands, versions, and argument layouts
// continue to run unchanged.
func IsBuiltInPIACP(agent AgentConfig) bool {
	return agent.ID == "pi" && agent.Command == "npx" && slices.Equal(agent.Args, []string{"-y", PiACPPackageSpec})
}
