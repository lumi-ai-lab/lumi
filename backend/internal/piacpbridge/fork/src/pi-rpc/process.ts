...
type SpawnParams = {
  ...
  sessionPath?: string
  /** System instructions appended through pi's native CLI option. */
  systemPromptAppend?: string
  /** Test override for the profile-bearing PI RPC handshake timeout. */
  handshakeTimeoutMs?: number
}

function createSystemPromptFile(text: string): { path: string; cleanup: () => void } {
    ...
    return {
      ...
      cleanup: () => {
        try {
          ...
          // Best-effort cleanup; never include the instruction body in an error.
        }
      }
    }
  } catch (error) {
...
export class PiRpcProcess {
  ...
  private constructor(child: ChildProcessWithoutNullStreams) {
    ...
    // A write can emit an error on the stdin Socket in addition to invoking its
    // callback. Keep a permanent listener so a dispose/write race becomes a
    // rejected RPC request instead of an uncaught EPIPE that kills the adapter.
  ...
  static async spawn(params: SpawnParams): Promise<PiRpcProcess> {
    ...
    try {
      try {
        const state = (await (systemPromptFile
          ? withTimeout(proc.getState(), params.handshakeTimeoutMs ?? 30_000)
          : proc.getState())) as any
        ...
        // A profile-bearing process must prove that PI reached the RPC handshake
        // before the temporary file can be removed. Continuing after a failed
        // handshake would make instruction delivery unverifiable.
        if (systemPromptFile) {
          proc.dispose()
      ...
      // Reaching the RPC handshake means pi has parsed the CLI option and read
      // the file. Remove the sensitive material immediately.
      systemPromptFile?.cleanup()
    }
