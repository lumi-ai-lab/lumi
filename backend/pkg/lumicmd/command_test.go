...
func TestWeComRunRequesterConfigFlagOverridesEnvironment(t *testing.T) {
	...
	writePolicy := func(name, userID string) string {
		...
		raw := fmt.Sprintf(`{"version":2,"botId":"bot-123","users":[{"userId":%q,"displayName":"User","enabled":true,"authorization":{"capabilities":["com.example.reports.read"],"claims":{"com.example.reports":{"tenantIds":["tenant-a"]}}}}]}`, userID)
