...
- RequesterContext 会自动写入 Agent 运行时目录，并通过 `LUMI_REQUESTER_CONTEXT_DIR` 传给 Agent；目录和临时会话文件分别使用 `0755` 与 `0644`，无需额外 root、UID/GID 或 reader group 配置。通用的 `runAsUid`、`runAsGid` 和 `supplementaryGids` 仍可用于其他进程隔离需求，但不再是 requester 权限链路的前置条件。
