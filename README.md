...
- PI 0.83.0 的 RPC 模式默认不会交互式询问项目信任，因此未预先信任的 Workspace 会忽略项目级 `.pi/settings.json`、extensions 和 skills。只有部署方已独立评审并信任固定 Workspace 时，才可在对应 Pi Agent 的 `env` 中设置 `PI_ACP_APPROVE_PROJECT=true`；Lumi 会为每个 PI RPC 子进程传递原生 `--approve`。未设置时保持 PI 的默认项目信任行为，非法值会拒绝启动。
- 当前 `pi-acp` 会接收 ACP `mcpServers` 但不会把它们接入 PI，因此 Lumi MCP SSOT 暂不对 PI 生效；PI skills 会通过 `~/.pi/agent/skills`、`<workspace>/.pi/skills` 生效。
- Linux 安全部署可为 Pi 配置可选的 `runAsUid`、`runAsGid` 和 `supplementaryGids`。UID/GID 必须由部署解析，`runAsUid`/`runAsGid` 必须成对出现且都不能为 root，Pi UID 还必须不同于 Lumi publisher UID；配置后子进程只获得声明的 supplementary groups。配合成对设置 `LUMI_REQUESTER_CONTEXT_ROOT=/run/lumi/requester-context` 与 `LUMI_REQUESTER_CONTEXT_READER_GID=<专用组 GID>`，RequesterContext 会写入 `<root>/<workspace>/pi`，context root、Workspace 和 Pi 目录及文件分别为 `0710`、`0640`。托管 Sandbox 会把每个 Workspace 独立的 `$LUMI_HOME/runtime/sandboxes/<workspace>/requester-context` 挂载到容器内 `/run/lumi/requester-context`，不会复用可由 Agent 控制的 `/lumi/runtime` 挂载。reader GID 必须出现在 Pi 的 primary 或 supplementary groups 中；安全模式下一个 Pi 进程不能跨 Workspace 复用。启动方需要具备设置 UID/GID/groups 的系统权限；Windows 不支持配置 run-as identity。
