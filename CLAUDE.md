...
```bash
cd backend
go build -o lumi ./cmd/lumi              # Build web server binary
go run ./cmd/lumi                          # Run with embedded web
go run ./cmd/lumi -web ../web/dist         # Run with external web dir
go run ./cmd/lumi -port 8080               # Custom port (default: 3000)
```

### Desktop App (backend/)
```bash
cd backend
go build -o lumi-desktop ./cmd/desktop     # Build desktop tray app
```

### Full Build (embedded single binary)
```bash
# Build web assets and embed into Go binary
cd web && npm run build && cd ../backend && go build -o lumi ./cmd/lumi
```
...
2. Backend stores file in `.lumi-uploads/` directory in workspace
...
| Path | Purpose |
|------|---------|
| `backend/cmd/lumi/main.go` | Web server entry point, embeds web assets |
...
1. `./lumi.config.json`
2. `./lumi.json`
3. `~/.lumi/lumi.config.json` (auto-created on first run)
...
### Backend Development
- Session data stored in `~/.config/lumi/sessions/`
- Uploaded files stored in `<workspace>/.lumi-uploads/`
